...
# TUMOR TYPE
pdf(paste0(fig_dir, "edf7a_care_tumor_type_malignant_abundance.pdf"), width = 3.5, height = 3, useDingbats = FALSE, bg = "transparent")
...
pdf(paste0(fig_dir, "edf8a_init_v_recur_malignant_abundance.pdf"), width = 3.5, height = 3, useDingbats = FALSE, bg = "transparent")
ggplot(mal_care_md_state_summary %>% 
  ...
  geom_line(aes(group=patient_id), color="gray85", linetype=2, size = 0.6) +
  geom_point(size = 0.6) +
  plot_theme +
...
pdf(paste0(fig_dir, "fig3d_genetic_alt_init_v_recur_malignant_abundance.pdf"), width = 3.5, height = 3, useDingbats = FALSE, bg = "transparent")
ggplot(mal_care_md_state_summary %>% 
...
pdf(paste0(fig_dir, "edf8a_init_v_recur_malignant_abundance.pdf"), width = 3.5, height = 3, useDingbats = FALSE, bg = "transparent")
ggplot(mal_care_md_state_summary %>% 
         filter(timepoint!="T3", case_barcode%in%true_primaries$case_barcode, idh_codel_subtype=="Astro.", cell_state%in%c("AC-like", "MES-like", "Undifferentiated", "Cycling")), aes(x = sample_time, y = freq*100)) + 
  geom_boxplot(outlier.shape = NA, aes(fill=cell_state)) +
  geom_line(aes(group=patient_id), color="gray85", linetype=2, size = 0.6) +
  geom_point(size = 0.6) +
  plot_theme +
...
pdf(paste0(fig_dir, "edf8b_init_v_recur_tumor_type_malignant_abundance.pdf"), width = 3.5, height = 3, useDingbats = FALSE, bg = "transparent")
ggplot(mal_care_md_state_summary %>% 
         filter(timepoint!="T3", cell_state%in%c("AC-like", "MES-like", "Undifferentiated", "Cycling")), aes(x = sample_time, y = freq*100)) + 
  geom_boxplot(outlier.shape = NA, aes(fill=cell_state)) +
  geom_line(aes(group=patient_id), color="gray85", linetype=2, size = 0.6) +
...
gg_grade <- ggplot(mal_care_md_state_summary_heatmap %>% 
  ...
  theme(
    ...
    legend.title = element_text(size = 7),
    legend.text  = element_text(size = 7),
    legend.key.size = unit(4, "pt"),      # <– shrink legend color boxes
    legend.spacing.x = unit(1, "pt"),     # optional: tighten spacing
    legend.spacing.y = unit(1, "pt")
  )


pdf(paste0(fig_dir, "fig3a_cell_state_abundance_heatmap.pdf"), width = 4, height = 3, useDingbats = FALSE)
egg::ggarrange(gg_heatmap, gg_grade, nrow = 2, heights = c(0.6, 0.1))
dev.off()
...
ggplot(res, aes(x=`AC-like`, y=Undifferentiated)) +
  ...
  plot_theme
cor.test(res$`AC-like`, res$Undifferentiated, method = "s")

ggplot(res, aes(x=Cycling, y=Undifferentiated)) +
  geom_point() +
  stat_cor(method="spearman", cor.coef.name = "rho") +
  plot_theme
cor.test(res$Cycling, res$Undifferentiated, method = "s")
